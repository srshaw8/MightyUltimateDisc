...
// submitForm()
//  Modified: Steve Shaw
//  Process submit from page; populate hidden form field with list of 
//	captains / players that were selected for team or owners that were 
//	selected for event
// -------------------------------------------------------------------
function submitForm(fields) {
	...
	for (i=0; i < fields.length; i++) { 
		data = '';
		idList = document.getElementById(fields[i]).options;
		lenID = idList.length;
	    for(j=0; j<lenID; j++) {
	    	if (data == '') {
    			data += idList.item(j).value;
    		} else {
            	data += ',';
	        	data += idList.item(j).value;
    		}
	    }
        if (fields[i] == 'selectedPlayers') {
    		document.selectionForm.Player_List.value = data;
		} else if (fields[i] == 'selectedOwners') {
			document.selectionForm.Owner_List.value = data;
		} else if (fields[i] == 'selectedCaptains') {
    		document.selectionForm.Captain_List.value = data;
    	}
    }
